...
walkLeft = [playerLeft]
walkRight2 =  [pygame.transform.flip(player2Left,30,0)]
walkLeft2 = [player2Left]
PlayTime= 90    #time limit for level 1
...
multiplayer = False
bonusDrop = False
level2_active = False   #keeping track if level 2 is active
...
yellowBall = pygame.image.load('yball3.png')
smallY = pygame.transform.scale(yellowBall,(30,30))
yellowBall = pygame.transform.scale(yellowBall,(80,80))
...
class Player:
    ...
    def hit(self):      #Hitbox Code
        if(totalLives == 0 or totalLives2 == 0):
            ...
            win.blit(gameOver, (300,150))
            msg1 = font.render("Press Q to Quit",5,(0,0,0))
            win.blit(msg1,(300,180))
...
class lives:
    ...
    def drawlives2(self,win,totalLives2):
...
class Bubble:
    ...
    def drawBubble(self,win):
        if self.visible:
            if level2_active:
                win.blit(yellowBall,(self.pos_x,self.pos_y))
                self.pos_x+= self.spd_x
                self.pos_y+= self.spd_y

                if(self.pos_y > 286 or self.pos_y < 0):             #checking if bubble hits borders of screen
                    self.spd_y= self.spd_y * -1
                if self.pos_x > 625 or self.pos_x < 5:
                    self.spd_x = self.spd_x * -1
                self.hitbox = (self.pos_x+2,self.pos_y+2,70,70)
            else:
...
class smallBubble:
    ...
    def drawSmall(self,win):
        if self.visible1:
            if level2_active:
                win.blit(smallY,(self.pos_x,self.pos_y))  #displaying bubble image on screen

                self.pos_x+= self.spd_x
                self.pos_y+= self.spd_y

                if(self.pos_y > 338 or self.pos_y < 0):             #checking if bubble hits borders of screen
                    self.spd_y= self.spd_y * -1
                if self.pos_x > 660 or self.pos_x < 5:
                    self.spd_x = self.spd_x * -1
                self.hitbox = (self.pos_x,self.pos_y,30,30)
            else:
...
def redrawGameWindow():
    global level2_active,menu_active
    if not level2_active:
        win.blit(bg, (0,0))
        for i in arrow:
            i.draw(win)
        for i in arrow2:
        ...
        if bubbleList==[] and ballList==[]:
            ...
            if len(arrow2) >= 1:
                for i in arrow2:
                    arrow2.pop(arrow2.index(i))
            level2_active = True
            ballList.append(Bubble(randint(20,100),130))
            pygame.display.update()
            i = 0
            while i < 300:
                pygame.time.delay(10)
...
buttons = [singlePlayer,multiPlayer,control]
menu_active= True
